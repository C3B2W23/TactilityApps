...
## Build Troubleshooting (Dec 26, 2024)

### Summary
Getting a standalone MesholaMessenger app ELF to link required custom HAL glue, weak stubs, and a larger partition/flash profile. The final firmware (`MesholaMessenger.bin`) is ~1.07 MB and fits with `single_app_large` + 16MB flash. Key fixes are captured below so we can reproduce quickly.

### Common blockers and fixes
- **ESP-IDF / Python mismatch:** Use ESP-IDF 5.5; Python 3.12 works. Always `source $IDF_PATH/export.sh` in the same shell before `idf.py`.
- **TACTILITY_SDK_PATH not seen by CMake:** Export `TACTILITY_SDK_PATH` in the build shell before running `idf.py build`.
- **Missing submodules (cJSON, minitar, openthread junk files):** `git submodule update --init --recursive` after cleaning problematic submodule dirs (occasionally needed `git clean -fdx` in nested submodules).
- **RadioLib API change:** `Module` now requires a `RadioLibHal*`. Added `Esp32S3Hal` (see `protocol/Esp32S3Hal.h/.cpp`) and wired SX1262 T-Deck pins in `MeshCoreProtocol`.
- **ESP-IDF HAL incompatibility for S3:** Avoided RadioLib’s `hal/ESP-IDF/EspHal.h` (ESP32-only) by providing our own S3 HAL.
- **Linker missing esp_http_server/FreeRTOS symbols for app ELF:** Added weak stubs under `main/Source/compat/` for:
  - http server/client (esp_http_server / esp_http_client)
  - FreeRTOS event groups & timers
  - esp_now / esp_wifi / esp_netif (includes WIFI_EVENT/IP_EVENT bases)
  - TinyUSB MSC, VFS FAT info
  - minmea, cJSON helpers, LVGL screenshot
  - I2C driver hooks
  - ELF loader (`esp_elf_init/relocate/request/deinit`)
  These are link-time placeholders only; full firmware overrides them.
- **Partition too small:** Switched to `single_app_large` and flash size 16MB in `sdkconfig` to fit the ~1.07 MB binary.
- **Default channel embed:** Protocol exposes MeshCore Public channel by default (Hex `8b3387e9c5cdea6ac9e5edbaa115cd72`, Base64 `izOH6cXN6mrJ5e26oRXNcg==`).
...
### Artifact sizes (current)
- `MesholaMessenger.bin`: ~1.07 MB (0x10dbd0)
- Partition profile: `single_app_large`, flash size 16MB

### Where the shims live
- `main/Source/compat/` — weak stubs for httpd/http client/wifi/esp-now/timers/i2c/elf/minmea/cJSON/LVGL screenshot/VFS FAT/TinyUSB.
- `main/Source/protocol/Esp32S3Hal.*` — custom RadioLib HAL for ESP32-S3.
