...
void ChatView::createWelcomeView() {
    ...
    // Use default small font (only font enabled in lv_conf)
    ...
    if (_service) {
        ...
        lv_obj_set_style_text_font(nodeInfo, &lv_font_montserrat_14, LV_STATE_DEFAULT);
    }
}
...
void ChatView::createConversationView() {
    ...
    if (!_headerBar) {
        ...
        lv_obj_set_style_text_font(_headerStatus, &lv_font_montserrat_14, LV_STATE_DEFAULT);
...
void ChatView::createMessageBubble(const Message& msg) {
    ...
    if (!isOutgoing && _hasActiveChannel && strlen(msg.senderName) > 0) {
        ...
        lv_obj_set_style_text_font(senderLabel, &lv_font_montserrat_14, LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_text_font(timeLabel, &lv_font_montserrat_14, LV_STATE_DEFAULT);
    ...
    if (isOutgoing) {
        ...
        lv_obj_set_style_text_font(statusIcon, &lv_font_montserrat_14, LV_STATE_DEFAULT);
    ...
    if (!isOutgoing && msg.rssi != 0) {
        ...
        lv_obj_set_style_text_font(rssiLabel, &lv_font_montserrat_14, LV_STATE_DEFAULT);
